...
O cliente participará ativamente na revisão de artefatos concretos em diferentes momentos do ciclo de desenvolvimento:
...
- **Protótipos Rápidos:** Objeto central do ciclo de validação visual no RAD. Os protótipos são construídos iterativamente no Figma e apresentados ao cliente a cada ciclo, permitindo que ele valide se a interface proposta reflete corretamente os fluxos reais de uso antes a implementação por meio de linhas de código. 
