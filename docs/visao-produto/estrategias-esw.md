...
| Limitações principais | Menor adequação a sistemas de grande escala ou missão crítica; alta dependência da disponibilidade do usuário; Risco de foco excessivo em interfaces e funcionalidades visíveis                                    | Mais complexo e prescritivo que outras abordagens ágeis; requer compromisso organizacional significativo                                                       |
...
O RAD foi escolhido por equilibrar dois aspectos centrais do FutBoard: a necessidade de entregar, dentro de um único semestre, um produto fortemente dependente de validação visual (dashboards e relatórios), e a realidade da equipe que conta com um único cliente, de fácil acesso e com disponibilidade constante.

Destaca-se alguns motivos:

- **Cliente único e colaboração contínua:** o prazo exige validação intensa com o técnico Marcus Vinicius, único ponto de contato e de decisão do produto. A fase de design do usuário do RAD viabiliza essa colaboração direta (WhatsApp/Teams), sem exigir os papéis de negócio formalizados que o DSDM demandaria para representar a mesma perspectiva do usuário.

- **Prototipagem como núcleo de validação:** o FutBoard tem como ponto central dashboards e relatórios que precisam ser lidos rapidamente por um usuário não-técnico durante a rotina do treino. O papel central da prototipagem no RAD permite que essas telas evoluam iterativamente até se tornarem o próprio sistema final, servindo simultaneamente como especificação e validação junto ao cliente, garantindo a satisfação do usuário com o produto final, algo que um processo com prototipagem apenas complementar dificultaria.
